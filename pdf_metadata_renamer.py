...
def show_config():
    ...
    for c in (0, 1):
        cfg_win.grid_columnconfigure(c, weight=(1 if c == 1 else 0))

    r = 0
    tk.Label(cfg_win, text='Output Pattern (papers):').grid(row=r, column=0, sticky='e', padx=5, pady=5)
    pat_entry = tk.Entry(cfg_win, width=60)
    pat_entry.insert(0, OUTPUT_PATTERN)
    pat_entry.grid(row=r, column=1, sticky='we', padx=5, pady=5); r += 1

    tk.Label(cfg_win, text='Book Output Pattern:').grid(row=r, column=0, sticky='e', padx=5, pady=5)
    book_pat_entry = tk.Entry(cfg_win, width=60)
    book_pat_entry.insert(0, BOOK_OUTPUT_PATTERN)
    book_pat_entry.grid(row=r, column=1, sticky='we', padx=5, pady=5); r += 1

    tk.Label(cfg_win, text='Unpublished Placeholder:').grid(row=r, column=0, sticky='e', padx=5, pady=5)
    plc_entry = tk.Entry(cfg_win, width=60)
    plc_entry.insert(0, UNPUBLISHED_PLACEHOLDER)
    plc_entry.grid(row=r, column=1, sticky='we', padx=5, pady=5); r += 1

    # Separate author formats
    tk.Label(cfg_win, text='Author Format (papers):').grid(row=r, column=0, sticky='e', padx=5, pady=5)
    paper_fmt_entry = tk.Entry(cfg_win, width=60)
    paper_fmt_entry.insert(0, AUTHOR_FMT_PAPER)
    paper_fmt_entry.grid(row=r, column=1, sticky='we', padx=5, pady=5); r += 1

    tk.Label(cfg_win, text='Author Format (books):').grid(row=r, column=0, sticky='e', padx=5, pady=5)
    book_fmt_entry = tk.Entry(cfg_win, width=60)
    book_fmt_entry.insert(0, AUTHOR_FMT_BOOK)
    book_fmt_entry.grid(row=r, column=1, sticky='we', padx=5, pady=5); r += 1

    tk.Label(cfg_win, text='Gemini API Key:').grid(row=r, column=0, sticky='e', padx=5, pady=5)
    api_entry_cfg = tk.Entry(cfg_win, width=60, show='*')
    api_entry_cfg.insert(0, API_KEY)
    api_entry_cfg.grid(row=r, column=1, sticky='we', padx=5, pady=5); r += 1

    tk.Label(cfg_win, text='Model:').grid(row=r, column=0, sticky='e', padx=5, pady=5)
    model_entry = tk.Entry(cfg_win, width=60)
    model_entry.insert(0, MODEL_NAME)
    model_entry.grid(row=r, column=1, sticky='we', padx=5, pady=5); r += 1

    # ── Embedded Cheatsheet (in the same Settings window) ───────────────────
    cheat_frame = tk.LabelFrame(cfg_win, text='Cheatsheet')
    cheat_frame.grid(row=r, column=0, columnspan=2, sticky='nsew', padx=6, pady=(8, 4))
    cfg_win.grid_rowconfigure(r, weight=1)  # let cheatsheet expand
    r += 1

    cheat_text = scrolledtext.ScrolledText(cheat_frame, width=90, height=10)
    cheat_text.insert('1.0',
        "Tokens for filename patterns:\n"
        "  {journal}  {year}  {authors}  {title}\n"
        "\n"
        "Author tokens (used inside Author Format):\n"
        ...
        "\n"
        "Notes:\n"
        "- {journal} is the publisher for books.\n"
        "- You control commas and dots in your author format.\n"
        "- Authors are joined by ', '.\n"
    )
    cheat_text.config(state='disabled')
    cheat_text.pack(fill='both', expand=True, padx=6, pady=6)
    # ────────────────────────────────────────────────────────────────────────
    ...
    btn_frame = tk.Frame(cfg_win)
    btn_frame.grid(row=r, column=1, sticky='e', padx=5, pady=10)
    tk.Button(btn_frame, text='Save', command=save_and_close).pack(side='right')
